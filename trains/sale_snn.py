...
lay1 = tf.nn.relu(ml.layer_basic(ml.bn(x), 2048))
lay2 = tf.nn.relu(ml.layer_basic(tf.nn.dropout(ml.bn(lay1), keep_prob=keep_prob), 256))
lay3 = tf.nn.relu(ml.layer_basic(tf.nn.dropout(ml.bn(lay2), keep_prob=keep_prob), 16))
lay4 = tf.nn.relu(ml.layer_basic(tf.nn.dropout(ml.bn(lay3), keep_prob=keep_prob), 4))
...
for i in range(10 ** 10):
    ...
    sess.run(optimizer, feed_dict={x: a, y_: b, keep_prob: 1})
    if i % 100 == 0:
        a_test, b_test = next(data=data_test)
        y_train, y_train_, loss_train = sess.run((y, y_, loss), feed_dict={x: a, y_: b, keep_prob: 1})
        y_test, y_test_, loss_test = sess.run((y, y_, loss), feed_dict={x: a_test, y_: b_test, keep_prob: 1})
